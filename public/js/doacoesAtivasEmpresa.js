/* arquivo: public/js/doacoesAtivasEmpresa.js - script do frontend: funcionalidades relacionadas a doacoesativasempresa - funções/constantes: preencherTabelaComDoacoes, doacoes, paginationControlsId, tr, email_usuario */
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function carregarUsuario() {
    try {
      ...
      const tableId =
        tipo_usuario === "empresa" ? "doacoesTableEmpresa" : "doacoesTableOng";
      const searchInputId =
        tipo_usuario === "empresa" ? "searchInputEmpresa" : "searchInputOng";
      const totalItensId =
        tipo_usuario === "empresa" ? "totalItensEmpresa" : "totalItensOng";
      const totalPaginasId =
        tipo_usuario === "empresa" ? "totalPaginasEmpresa" : "totalPaginasOng";
      const paginationControlsId =
  ...
  async function carregarDoacoesUsuario(email, tipo) {
    try {
      ...
      if (tipo === "empresa") {
        res = await fetch(
          `/doacoesConcluidasEmpresa/doacoesEmpresa?email=${encodeURIComponent(email)}`,
        );
      } else {
        res = await fetch(
          `/doacoesConcluidasONG/doacoesONG?email=${encodeURIComponent(email)}`,
        );
      }
