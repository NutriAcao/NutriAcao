...
export async function cadastrarResponsavelLegal(req, res) {
    ...
    if (!nome || !cpf || !email || !empresa_id) {
        return res.status(400).send("Dados essenciais do Responsável Legal (Nome, CPF, Email) ou o ID da Empresa não foram fornecidos.");
    }
    ...
    try {
        ...
        if (responsavelError) {
            console.error('Erro ao cadastrar Responsável Legal:', responsavelError.message);
        ...
        if (relacaoError) {
            console.error('Erro ao vincular Responsável à Empresa:', relacaoError.message);
            
            return res.status(500).send("Falha ao vincular Responsável à Empresa. O cadastro do responsável foi concluído, mas a vinculação falhou.");
        }

        
        return res.status(201).json({ 
            status: 'OK', 
            message: 'Responsável Legal cadastrado e vinculado com sucesso à Empresa!', 
            responsavel_id: responsavel_id,
            empresa_id: empresa_id 
        });
